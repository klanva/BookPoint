...
bool estimateFinishDateFromDailyPace(const BookReadingStats& stats, const ReadingStatsDateTime& today,
  ...
  if (!today.isValid() || !stats.startDate.isValid() || estimatedReadingSeconds == 0 ||
      stats.totalReadingSeconds == 0 || stats.isFinished) {
    return false;
  }
  ...
  // Convert remaining reading time into calendar time using the book's average
  // reading seconds per calendar day.
...
void formatFinishDate(const ReadingStatsDate& date, char* buf, const size_t len) {
  if (!buf || len == 0) return;
  if (!date.isValid()) {
    snprintf(buf, len, "-");
    return;
  }
  snprintf(buf, len, "%02u.%02u.%04u", static_cast<unsigned>(date.day), static_cast<unsigned>(date.month),
