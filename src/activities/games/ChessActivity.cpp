      ...
      if (piece != EMPTY) {
        ...
        if (sprite) {
          if (isSelected) {
            ...
            // BookPoint GfxRenderer has no transparent draw: paint a white cell
            // behind the piece sprite so its background blends with light squares.
            renderer.fillRect(cx, cy, CELL, CELL, false);
            renderer.drawImage(sprite, cx, cy, CELL, CELL);
          }
        }
      }
