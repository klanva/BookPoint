...
void WifiSelectionActivity::checkConnectionStatus() {
  ...
  if (status == WL_CONNECTED) {
    ...
    // Keep the clock accurate. X3 has a real DS3231-class RTC (~2 ppm), so one
    // sync is enough. X4 has no hardware RTC — its software clock is backed by
    // the ESP32's internal RC oscillator, which drifts minutes per day — so it
    // must re-sync from NTP on every (re)connect. Both paths set the system
    // clock; X3 additionally writes the DS3231. No extra battery cost: this only
    // fires while WiFi is already connected (e.g. to sync books / browse OPDS).
