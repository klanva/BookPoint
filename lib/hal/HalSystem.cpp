...
bool hasPanicStack() {
  return panicStack[0].sp != 0;
}

std::string getPanicStackSummary() {
  if (panicStack[0].sp == 0) return "";
  char buffer[96];
  snprintf(buffer, sizeof(buffer), "SP: 0x%08lX [0x%08lX 0x%08lX 0x%08lX 0x%08lX]",
           static_cast<unsigned long>(panicStack[0].sp),
           static_cast<unsigned long>(panicStack[0].spp[0]),
           static_cast<unsigned long>(panicStack[0].spp[1]),
           static_cast<unsigned long>(panicStack[0].spp[2]),
           static_cast<unsigned long>(panicStack[0].spp[3]));
